...
import { getFilterUrlMap } from './FunctionTool'
...
class Filter extends React.Component {
	...
	transformMultiSelect(field, key, getFieldDecorator) {
		...
		return <Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key, { initialValue: field.defaultValue })(
			<Select allowClear mode="multiple" optionFilterProp="children" filterOption={mySearchInfo} placeholder={field.placeholder || '请选择'} getPopupContainer={() => document.body}>{options}</Select>)}</FormItem></Col>
	}
	...
	transformSelect(field, key, getFieldDecorator) {
		...
		return <Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key,{initialValue: field.defaultValue,})(
			<Select allowClear showSearch placeholder={field.placeholder || '请选择'} optionFilterProp="children" filterOption={mySearchInfo} getPopupContainer={() => document.body}>{options}</Select>)}</FormItem></Col>
	}
	...
	transformTreeSelect(field, key, getFieldDecorator) {
		...
		const loop = (data, _parent) => data.map((item) => {
			...
			*/
			...
			return <TreeNode title={item.name} key={item.key} isLeaf={true} value={item.key} disabled={item.disabled || false}/>;
		})
		...
		return <Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} lg={{ span: 8 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key,{initialValue: field.defaultValue,})(
			<TreeSelect treeDefaultExpandAll onChange={moChange} getPopupContainer={() => document.body} style={{ width:'100%' }} size="default" dropdownStyle={{maxHeight: '300px',overFlow: 'scroll'}} placeholder={field.placeholder}>{treeNodes}
			</TreeSelect>)}</FormItem></Col>
	}
